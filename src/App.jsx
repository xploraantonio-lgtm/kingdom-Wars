...
import { TILE_TYPES, assignPlayerBase, generateMap, removeOldestGemTile, spawnGemTile } from './data/tileTypes'
...
export default function App() {
  ...
  function tileDescription(tile) {
    ...
    if (tile.type === 'rubble') return 'Casilla disponible para fundar una base'
    return 'Terreno del mundo'
  }

  function primaryLabel(tile) {
    if (tile.isPlayerBase) return 'Tu base'
    if (tile.type === 'rubble') return 'Fundar base'
  ...
  function handlePrimaryAction() {
    if (!selected) return
    if (selected.type !== 'rubble' || selected.isPlayerBase) return

    const result = assignPlayerBase(tiles, selected.id)
    if (!result.assigned) {
    ...
    setNotice(`Base fundada en (${selected.worldX}, ${selected.worldY}). Entorno: ${result.counts.wood} madera, ${result.counts.stone} piedra y ${result.counts.food} comida.`)
  }
      ...
      <section className="game-phone" aria-label="Kingdom Wars prototype">
        <header className="top-bar">
          <div className="brand-row">
            <div>
              <p className="eyebrow">TEMPORADA 0 · MAPA 25×25</p>
              <h1>KINGDOM WARS</h1>
            </div>
            ...
            {tiles.map((tile) => {
              ...
              return (
                <button
                  key={tile.id}
                  type="button"
                  className={`tile tile-${def.role} ${tile.type === 'gems' ? 'gem-spawn' : ''} ${tile.isPlayerBase ? 'player-base' : ''} ${onXAxis ? 'axis-x' : ''} ${onYAxis ? 'axis-y' : ''} ${isOrigin ? 'origin' : ''} ${selectedId === tile.id ? 'selected' : ''}`}
                  onClick={() => selectTile(tile)}
                  aria-label={`${def.name}, coordenadas ${tile.worldX}, ${tile.worldY}`}
                >
                  <TileImage def={def} />
                  {(onXAxis || onYAxis) && (
                  ...
                  {tile.isPlayerBase && (
                    <span className="base-marker" aria-hidden="true">🏰</span>
                  )}
                </button>
              )
            })}
        ...
        <section className="selection-panel">
          {selected ? (
            <>
              <div className="selection-icon">{selected.isPlayerBase ? '🏰' : TILE_TYPES[selected.type].fallback}</div>
              ...
              <button type="button" className="primary-action" onClick={handlePrimaryAction} disabled={selected.isPlayerBase}>
